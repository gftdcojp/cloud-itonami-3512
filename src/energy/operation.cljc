(ns energy.operation
  "OperationActor -- one community-energy operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Energy Advisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Grid Policy Governor (:govern)
  ...
  One graph run = one community-energy operation (intake -> advise ->
  ...
  {:status :approved}}` (or :rejected). `:actuation/dispatch-battery`/
  `:actuation/finalize-settlement` ALWAYS reach this node when the
  governor is clean -- see `energy.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [energy.energyadvisor :as energyadvisor]
            [energy.governor :as governor]
...
(defn build
  "Compiles an OperationActor graph bound to `store` (any `energy.
  store/Store`).
  opts:
    :advisor      -- an `energy.energyadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (energyadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Energy Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (energyadvisor/-advise advisor store request)]
            {:proposal p :audit [(energyadvisor/trace request p)]})))

      ;; Grid Policy Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
